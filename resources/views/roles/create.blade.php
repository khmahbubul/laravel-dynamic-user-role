                    ...
                    <form action="{{ route('roles.store') }}" method="post">
                        ...
                        @foreach ($permissions as $permission)
                            <div class="form-group form-check">
                                <input type="checkbox" value="{{ $permission->id }}" name="permission[]" class="form-check-input" id="exampleCheck{{ $permission->id }}" @if(is_array(old('permission')) && in_array($permission->id, old('permission'))) checked @endif>
